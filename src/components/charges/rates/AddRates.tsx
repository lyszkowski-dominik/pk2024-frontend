...
const EditableRates = ({
  ...
  const initialData = useGetRatesById(id || -1);
  const addRates = useAddRate(hoaId);
  const updateRates = useUpdateRate(hoaId, id || -1);

  const mutation = id ? updateRates : addRates;
  ...
  const formikProps: FormikWrapperProps<any> = {
    ...
    initialValues: {
      ...
      rates: initialData.data?.rates || initialValues.rates,
      hoa: hoaId,
    },
    validationSchema,
            ...
            <>
              <div
                className={`${styles.date} ${
                  errors.start && inputStyles.invalid
                }`}
              >
                <InputField
                  type="date"
